...
const Reports = () => {
  ...
  const enrichRealRevenueLogs = async (rawLogs: any[]): Promise<any[]> => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      if (isStaff) {
        ...
        const enrichedToday = await enrichRealRevenueLogs(todayRealRevLog);
        const mappedToday = enrichedToday.map(r => ({
          ...r,
          is_fake: false,
          revenue_date: todayStr,
          recorded_at: r.recorded_at || `${todayStr}T10:00:00.000Z`
        }));
  ...
  const handleExportExcel = async () => {
    ...
    if (isStaff) {
      ...
      const enrichedFresh = await enrichRealRevenueLogs(freshRev || []);
      finalExportItems = enrichedFresh.map(r => ({
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontSize: '0.9rem', fontWeight: '700', color: 'var(--text-main)' }}>
                                  {r.is_fake ? (
                                    ...
                                    <span>
                                      {r.service_name || (r.type === 'retail' ? 'Thu dịch vụ lẻ' : r.type === 'combo' ? 'Thu combo' : r.type === 'package_sale' ? 'Thu bán thẻ liệu trình' : 'Trừ buổi liệu trình')}
                                      {r.technician_name && r.technician_name !== '---' && (
                                        <span style={{ color: 'var(--primary)', marginLeft: '0.5rem', fontWeight: '600', fontSize: '0.8rem' }}>
                                          ({r.type === 'package_sale' ? 'Người bán' : 'KTV'}: {r.technician_name})
                                        </span>
                                      )}
                                    </span>
                                  )}
